...
namespace src.MVVM.ViewModels
{
    ...
    public class WeatherViewModel
    {
        ...
        private async Task GetWeather(Location location)
        {
            try
            {
                var url = $"https://api.open-meteo.com/v1/forecast?latitude={location.Latitude.ToString().Replace(",", ".")}&longitude={location.Longitude.ToString().Replace(",", ".")}&daily=weathercode,temperature_2m_max,temperature_2m_min&current_weather=true&timezone=America%2FChicago";
                IsLoading = true;
